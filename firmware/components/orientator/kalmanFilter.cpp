...
void kalmanFilter::stateUpdate(system_state_t& perdicted_state, const system_state_t& measured_state) {
    float K_A = perdicted_state.variance_angle / (perdicted_state.variance_angle + measured_state.variance_angle);
    float K_AV = perdicted_state.variance_velocity/ (perdicted_state.variance_velocity + measured_state.variance_velocity);
    ...
    perdicted_state.variance_angle *= 1-K_A;
    perdicted_state.variance_velocity*= 1-K_AV;
    perdicted_state.variance_acceleration *= pow(1-K_AV, 0.5); // honestly I have no idea what I'm doing here but it works
}
