...
bool IRAM_ATTR esc_rmt_rx_done_callback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *received_data, void *user_data)
{
    ...
    {
        for(int j=0; j<received_data->received_symbols[i].duration0/callback_data->rx_clock_div; j++)
        {
            encoded_value = (encoded_value<<1)|0;
            value_length++;
        }
        for(int j=0; j<received_data->received_symbols[i].duration1/callback_data->rx_clock_div; j++)
        {
