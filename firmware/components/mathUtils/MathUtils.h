...
// Vector scaling
template<typename T> vec2<T> operator * (vec2<T> const& vector, float const& scale) { return {(T)(vector.x * scale), (T)(vector.y * scale)}; }
template<typename T> vec2<T> operator * (float const& scale, vec2<T> const& vector) { return {(T)(vector.x * scale), (T)(vector.y * scale)}; }
template<typename T> vec3<T> operator * (vec3<T> const& vector, float const& scale) { return {(T)(vector.x * scale), (T)(vector.y * scale), (T)(vector.z * scale)}; }
template<typename T> vec3<T> operator * (float const& scale, vec3<T> const& vector) { return {(T)(vector.x * scale), (T)(vector.y * scale), (T)(vector.z * scale)}; }
