...
namespace MathUtils {
  ...
  angle angleFromUInt16(uint16_t LSB) { return angleFromRadians((float)LSB/(1<<15) * (float)M_PI); }
  ...
  uint16_t getUInt16(angle angle) { return (1<<15) * getRadians(angle) / (float)M_PI; }
